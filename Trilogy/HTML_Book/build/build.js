// build.js — assembles the HTML book chapter markdown into a single .docx manuscript
// Trade-paperback formatting, 6"×9", Garamond body, justified, page numbers.
// Handles fenced code blocks (```) as monospace blocks.
...
const OUTPUT = path.join(ROOT, 'What You Need to Know Before React and AI - HTML.docx');
...
const BOOK_SUBTITLE = 'HTML';
const AUTHOR = 'Elkanah Donkor';

const MANIFEST = [
  { file: 'chapter-00-introduction.md', label: 'Introduction', chapter: false },
  { file: 'chapter-01-first-file.md', label: 'Chapter 1', chapter: true },
  { file: 'chapter-02-anatomy.md', label: 'Chapter 2', chapter: true },
  { file: 'chapter-03-text.md', label: 'Chapter 3', chapter: true },
  { file: 'chapter-04-links-nav.md', label: 'Chapter 4', chapter: true },
  { file: 'chapter-05-images.md', label: 'Chapter 5', chapter: true },
  { file: 'chapter-06-semantic.md', label: 'Chapter 6', chapter: true },
  { file: 'chapter-07-forms.md', label: 'Chapter 7', chapter: true },
  { file: 'chapter-08-tables.md', label: 'Chapter 8', chapter: true },
  { file: 'chapter-09-accessibility.md', label: 'Chapter 9', chapter: true },
  { file: 'chapter-10-devtools.md', label: 'Chapter 10', chapter: true },
  { file: 'chapter-11-shipping.md', label: 'Chapter 11', chapter: true },
...
function blockize(md) {
  ...
  for (const rawLine of lines) {
    ...
    if (/^\s*\d+\.\s+/.test(line)) {
      flushPara();
      blocks.push({ type: 'li-num', text: line.replace(/^\s*\d+\.\s+/, '') });
      continue;
    }
    buf.push(line);
  }
...
function listItem(text, numbered = false) {
  return new Paragraph({
    ...
    children: [
      new TextRun({ font: FONT_BODY, size: 22, text: numbered ? '• ' : '• ' }),
...
function codeBlock(text) {
  // Each line as its own paragraph, monospace, slightly smaller.
...
function titlePage() {
  return [
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text: BOOK_TITLE, bold: true, font: FONT_HEAD, size: 44 })],
    }),
    ...blank(2),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text: BOOK_SUBTITLE, italics: true, font: FONT_HEAD, size: 36 })],
    }),
    ...blank(8),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text: AUTHOR, font: FONT_HEAD, size: 28 })],
    }),
    pageBreak(),
  ];
}
...
function copyrightPage() {
  const items = [
    ...
    'All rights reserved.',
    '',
    'Book 2 of the trilogy "What You Need to Know Before React and AI."',
...
function chapterOpen(label, title) {
  return [
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 480 },
      children: [new TextRun({ text: title, bold: true, font: FONT_HEAD, size: 36 })],
    }),
  ];
}
...
function renderChapterBody(blocks) {
  ...
  for (const b of blocks) {
    ...
    if (b.type === 'li') { out.push(listItem(b.text, false)); firstOfSection = true; continue; }
    if (b.type === 'li-num') { out.push(listItem(b.text, true)); firstOfSection = true; continue; }
